...
/*   Updated: 2023/02/05 14:02:33 by lboulatr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (is_charset(array[col->x_perso][col->y_perso + 1]) == 1)
	{
	...
	if (is_charset(array[col->x_perso + 1][col->y_perso]) == 1)
	{
		array[col->x_perso][col->y_perso] = '2';
		col->x_perso++;
		if (path_finder_c(g, col, array) == -1)
...
int	path_finder_c(t_game *g, t_col *col, char **array)
{
	if (col->x_perso == col->x && col->y_perso == col->y)
		return (1);
	if (is_charset(array[col->x_perso][col->y_perso]) == 1)
	{
